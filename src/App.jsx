...
const teams = [
  { id: 1, name: 'GEN', fullName: '젠지 (Gen.G)', power: 94, description: '안정적인 운영과 강력한 라인전', colors: { primary: '#D4AF37', secondary: '#000000' } }, // 골드 조정
  ...
  { id: 9, name: 'DRX', fullName: '디알엑스 (DRX)', power: 80, description: '변화를 추구하는 팀', colors: { primary: '#3848A2', secondary: '#000000' } }, // DRX 블루
...
const getOvrBadgeStyle = (ovr) => {
  if (ovr >= 90) return 'bg-red-100 text-red-700 border-red-300 ring-red-200'; // S급
  ...
  return 'bg-gray-100 text-gray-600 border-gray-300 ring-gray-200'; // C급
};
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-4xl font-black mb-8 text-gray-800 tracking-tight">LCK MANAGER 2026</h1>
        ...
        <button onClick={() => navigate('/new-league')} className="w-full mt-6 bg-white border-2 border-dashed border-gray-300 py-6 rounded-xl text-gray-400 hover:border-blue-500 hover:text-blue-500 font-bold text-xl transition flex items-center justify-center gap-2">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 transition-colors duration-500" style={{backgroundColor:`${current.colors.primary}10`}}>
      <div className="bg-white p-10 rounded-3xl shadow-2xl max-w-2xl w-full text-center border-t-8" style={{borderColor:current.colors.primary}}>
        <h2 className="text-3xl font-black mb-2">CHOOSE YOUR TEAM</h2>
        <div className="flex items-center justify-between mb-8 mt-8">
          ...
          <div className="flex flex-col items-center transform transition duration-300">
            <div className="w-40 h-40 rounded-full flex items-center justify-center text-5xl font-black text-white shadow-xl mb-6 ring-4 ring-white" style={{backgroundColor:current.colors.primary}}>
              {current.name}
            </div>
            <h3 className="text-3xl font-bold text-gray-800">{current.fullName}</h3>
            <div className="mt-3 inline-block bg-gray-100 px-4 py-1.5 rounded-full text-sm font-bold border border-gray-200">
              OVR POWER: <span className="text-blue-600 text-lg">{current.power}</span>
            </div>
          </div>
...
// --- Dashboard (Basketball GM Style Layout) ---
function Dashboard() {
  ...
  if (!league) return <div className="flex h-screen items-center justify-center font-bold text-gray-500">LOADING DATA...</div>;
  ...
  const menuItems = [
    ...
    { name: 'Standings', icon: '🏆' },
    { name: 'Schedule', icon: '📅' },
    { name: 'Free Agents', icon: '💸' },
    { name: 'Power Rankings', icon: '📈' },
    { name: 'History', icon: '📜' },
  ];
    ...
    <div className="flex h-screen bg-gray-100 overflow-hidden font-sans">
      ...
      <aside className="w-64 bg-gray-900 text-gray-300 flex-shrink-0 flex flex-col shadow-xl z-20">
        <div className="p-5 bg-gray-800 border-b border-gray-700 flex items-center gap-3">
          <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-white text-xs shadow-lg" style={{backgroundColor: myTeam.colors.primary}}>
            {myTeam.name}
          </div>
          <div>
            <div className="text-white font-bold text-sm leading-tight">{myTeam.fullName}</div>
            <div className="text-xs text-gray-400">GM Mode</div>
          </div>
        </div>
        
        <div className="flex-1 overflow-y-auto py-4">
          <div className="px-4 mb-2 text-xs font-bold text-gray-500 uppercase tracking-wider">League Menu</div>
          <nav className="space-y-1 px-2">
            {menuItems.map(item => (
              <button key={item.name} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-colors ${item.name === 'Dashboard' ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-gray-800 hover:text-white'}`}>
        ...
        <div className="p-4 border-t border-gray-700 bg-gray-800">
          <button onClick={() => navigate('/')} className="w-full flex items-center justify-center gap-2 text-xs font-bold text-gray-400 hover:text-white transition">
            <span>🚪</span> EXIT TO MAIN
          </button>
        </div>
      </aside>
        ...
        <header className="bg-white border-b h-14 flex items-center justify-between px-6 shadow-sm z-10 flex-shrink-0">
          <div className="flex items-center gap-6 text-sm">
            <div className="flex items-center gap-2 font-bold text-gray-700">
              <span className="text-gray-400">📅</span> 2026 Spring Week 1
            </div>
            <div className="h-4 w-px bg-gray-300"></div>
            <div className="flex items-center gap-2 font-bold text-gray-700">
              <span className="text-gray-400">🏆</span> 0승 0패 (1위)
            </div>
            <div className="h-4 w-px bg-gray-300"></div>
            <div className="flex items-center gap-2 font-bold text-green-600">
              <span className="text-gray-400">💰</span> ₩100.0M
            </div>
          </div>
          
          <button className="bg-green-600 hover:bg-green-700 text-white px-6 py-1.5 rounded-full font-bold text-sm shadow-sm transition flex items-center gap-2">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="grid grid-cols-12 gap-6 max-w-7xl mx-auto">
            ...
            <div className="col-span-12 lg:col-span-8 bg-white rounded-lg border shadow-sm p-5 relative overflow-hidden">
               ...
               <h3 className="text-lg font-bold text-gray-800 mb-2">Upcoming Match</h3>
               <div className="flex items-center justify-between bg-gray-50 rounded-xl p-6 border">
                  <div className="text-center w-1/3">
                    <div className="text-4xl font-black text-gray-800 mb-2">{myTeam.name}</div>
                    <div className="text-sm font-bold text-gray-500">0 - 0</div>
                  </div>
                  <div className="text-center w-1/3">
                    <div className="text-xs font-bold text-gray-400 uppercase">VS</div>
                    <div className="text-3xl font-bold text-gray-300 my-2">@</div>
                    <div className="text-xs font-bold text-blue-600">Week 1</div>
                  </div>
                  <div className="text-center w-1/3">
                    <div className="text-4xl font-black text-gray-300 mb-2">TBD</div>
                    ...
                    <tbody>
                      {teams.map((t, i) => (
                        <tr key={t.id} onClick={() => setViewingTeamId(t.id)} 
                            className={`cursor-pointer border-b last:border-0 ${viewingTeamId === t.id ? 'bg-blue-50' : 'hover:bg-gray-50'}`}>
            ...
            <div className="col-span-12 bg-white rounded-lg border shadow-sm flex flex-col min-h-[500px]">
              <div className="p-5 border-b flex justify-between items-center bg-gray-50 rounded-t-lg">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-2xl font-black text-gray-800">{viewingTeam.fullName}</h2>
                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wide">Roster Overview</p>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-xl font-black text-blue-600">{viewingTeam.power} <span className="text-xs text-gray-400">OVR</span></div>
                </div>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-white text-gray-400 text-xs uppercase font-bold border-b">
                    <tr>
                      ...
                      <th className="py-3 px-6 text-left">Stats</th>
                    </tr>
                  </thead>
                      ...
                      currentRoster.map((p, i) => (
                        <tr key={i} className="hover:bg-gray-50 transition duration-150 group">
                          ...
                          <td className="py-3 px-6 text-center">
                            {/* 1. 수정된 색상 로직 적용 */}
                            <span className={`inline-flex items-center justify-center w-10 h-8 rounded-lg font-black text-sm shadow-sm border ${getOvrBadgeStyle(p.종합)}`}>
                              {p.종합}
                            </span>
                          </td>
                          <td className="py-3 px-6">
                             <span className="inline-block bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded font-medium border border-gray-200">
                               {p.특성}
                             </span>
                          </td>
                          <td className="py-3 px-6">
                            <div className="flex gap-2 text-xs font-medium text-gray-400">
                              <span title="라인전">LAN <b className="text-gray-700">{p.라인전}</b></span>
                              <span title="한타">TF <b className="text-gray-700">{p.한타}</b></span>
                              <span title="운영">MAC <b className="text-gray-700">{p.운영}</b></span>
                            </div>
                          </td>
                        </tr>
                      ))
                    ) : (
                      <tr><td colSpan="5" className="py-20 text-center text-gray-300">No Roster Data</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
