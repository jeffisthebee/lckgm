...
const calculatePOS = (history, winningTeamName) => {
    ...
    history.forEach(game => {
        ...
        (targetPicks || []).forEach(p => {
            ...
            // [FIX] Use pogScore already computed by recalcPogForMatch (new formula).
            // Fall back to recalculating only for legacy data missing pogScore entirely.
            let score;
            if (p.pogScore != null) {
                score = p.pogScore;
            } else {
...
export default function DetailedMatchResultModal({ result, onClose, teamA, teamB }) {
    ...
    const posPlayer = useMemo(() => {
        // [FIX] Always recalculate from history picks so POS uses the new pogScore formula.
        // Never trust the stale saved result.posPlayer — it may have been computed with
        // the old formula, especially for games played before the formula update.
        if (isBo5) {
