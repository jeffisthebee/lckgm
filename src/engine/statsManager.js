...
export function computePlayoffAwards(league, teams) {
  ...
  // B. Finals MVP Logic [UPDATED FIX]
  // Use robust check to find the final match (matching AwardsTab logic)
  ...
  stats.playerRatings.forEach(player => {
      ...
      // Bonuses
      let bonusScore = 0;
      // FIX: Ensure loose comparison for player names
      const normalize = (n) => String(n || '').trim();
      const isFinalsMvp = finalsMvpName && normalize(finalsMvpName) === normalize(player.playerName);
      const isPogLeader = pogLeaderName && normalize(pogLeaderName) === normalize(player.playerName);
