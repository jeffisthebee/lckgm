...
export function runGameTickEngine(teamBlue, teamRed, picksBlue, picksRed, simOptions) {
  ...
  export const generateSchedule = (baronIds, elderIds) => {
    ...
    // Round 4: Super Week (5 matches) -> NOW BASED ON DRAFT ORDER (Offset 0)
    const allMatches = [];
    const n = 5; 
    ...
    for (let r = 0; r < 5; r++) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            allMatches.push({
                ...
                t1: t1,
                t2: t2,
                type: isSuperWeek ? 'super' : 'regular', // Round 4 is Super Week
                status: 'pending',
                format: isSuperWeek ? 'BO5' : 'BO3',     // Super Week is BO5
