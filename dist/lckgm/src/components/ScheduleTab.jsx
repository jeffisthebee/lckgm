    ...
    useEffect(() => {
        ...
        const simPlayoffMatch = (id) => {
            ...
            if (targetLeague === 'LPL') {
                ...
                const lcpPo3 = playoffs.find(m => m.id === 'lcp_po3');
                assignT1(lcpPo3, getSeedId(1)); assignT2(lcpPo3, r1m1.winnerId);
                const r2m1 = simPlayoffMatch('lcp_po3');

                const lcpPo4 = playoffs.find(m => m.id === 'lcp_po4');
                assignT1(lcpPo4, getSeedId(2)); assignT2(lcpPo4, r1m2.winnerId);
                const r2m2 = simPlayoffMatch('lcp_po4');

                const lcpPo5 = playoffs.find(m => m.id === 'lcp_po5');
                assignTeam(lcpPo5, r2m1?.winnerId, r2m2?.winnerId);
                const r3m1 = simPlayoffMatch('lcp_po5');

                const lcpPo6 = playoffs.find(m => m.id === 'lcp_po6');
                assignTeam(lcpPo6, r2m1?.loserId, r2m2?.loserId);
                const r2lm1 = simPlayoffMatch('lcp_po6');

                const lcpPo7 = playoffs.find(m => m.id === 'lcp_po7');
                assignTeam(lcpPo7, r2lm1?.winnerId, r3m1?.loserId);
                const r3lm1 = simPlayoffMatch('lcp_po7');

                const lcpPo8 = playoffs.find(m => m.id === 'lcp_po8');
                assignTeam(lcpPo8, r3m1?.winnerId, r3lm1?.winnerId);
                simPlayoffMatch('lcp_po8');

            } else if (targetLeague === 'CBLOL') {
                const cblolPi1Match = playoffs.find(m => m.id === 'cblol_pi1');
                assignTeam(cblolPi1Match, getSeedId(7), getSeedId(8));
                const cblolPi1 = simPlayoffMatch('cblol_pi1');

                const cblolPi2Match = playoffs.find(m => m.id === 'cblol_pi2');
                assignTeam(cblolPi2Match, getSeedId(5), getSeedId(6));
                const cblolPi2 = simPlayoffMatch('cblol_pi2');

                const cblolPi3Match = playoffs.find(m => m.id === 'cblol_pi3');
                assignTeam(cblolPi3Match, cblolPi1.winnerId, cblolPi2.loserId);
                const cblolPi3 = simPlayoffMatch('cblol_pi3');

                const cblolPo1Match = playoffs.find(m => m.id === 'cblol_po1');
                assignT1(cblolPo1Match, getSeedId(3));
                if (cblolPi2.winnerId && cblolPi3.winnerId && !cblolPo1Match.t2) {
                    const qual1 = cblolPi3.winnerId;
                    const qual2 = (cblolPi2.winnerId !== cblolPi3.winnerId) ? cblolPi2.winnerId : null;
                    cblolPo1Match.t2 = qual1;
                    isUpdated = true;
                    const cblolPo2Match2 = playoffs.find(m => m.id === 'cblol_po2');
                    if (qual2 && cblolPo2Match2 && !cblolPo2Match2.t2) { cblolPo2Match2.t2 = qual2; isUpdated = true; }
                }
                const cblolPo1 = simPlayoffMatch('cblol_po1');

                const cblolPo2Match = playoffs.find(m => m.id === 'cblol_po2');
                assignT1(cblolPo2Match, getSeedId(4));
                if (cblolPi2.winnerId && cblolPi3.winnerId && !cblolPo2Match.t2) {
                    cblolPo2Match.t2 = (cblolPi2.winnerId !== cblolPi3.winnerId) ? cblolPi2.winnerId : cblolPi3.winnerId;
                    isUpdated = true;
                }
                const cblolPo2 = simPlayoffMatch('cblol_po2');

                const cblolPo3Match = playoffs.find(m => m.id === 'cblol_po3');
                assignT1(cblolPo3Match, getSeedId(1));
                const cblolPo4Match = playoffs.find(m => m.id === 'cblol_po4');
                assignT1(cblolPo4Match, getSeedId(2));
                if (cblolPo1.winnerId && cblolPo2.winnerId && !cblolPo3Match.t2) {
                    const pickLower = Math.random() < 0.90;
                    cblolPo3Match.t2 = pickLower ? cblolPo2.winnerId : cblolPo1.winnerId; 
                    cblolPo4Match.t2 = pickLower ? cblolPo1.winnerId : cblolPo2.winnerId; 
                    isUpdated = true;
                }
                const cblolPo3 = simPlayoffMatch('cblol_po3');
                const cblolPo4 = simPlayoffMatch('cblol_po4');

                const cblolPo5Match = playoffs.find(m => m.id === 'cblol_po5');
                assignTeam(cblolPo5Match, cblolPo3.winnerId, cblolPo4.winnerId);
                const cblolPo5 = simPlayoffMatch('cblol_po5'); 

                const cblolPo6Match = playoffs.find(m => m.id === 'cblol_po6');
                assignTeam(cblolPo6Match, cblolPo1.loserId, cblolPo2.loserId);
                const cblolPo6 = simPlayoffMatch('cblol_po6');

                const cblolPo7Match = playoffs.find(m => m.id === 'cblol_po7');
                if (cblolPo7Match && cblolPo6.winnerId && cblolPo3.loserId && cblolPo4.loserId && !cblolPo7Match.t2) {
                    cblolPo7Match.t1 = cblolPo6.winnerId; cblolPo7Match.t2 = cblolPo4.loserId; isUpdated = true;
                }
                const cblolPo7 = simPlayoffMatch('cblol_po7');

                const cblolPo8Match = playoffs.find(m => m.id === 'cblol_po8');
                if (cblolPo8Match && cblolPo7.winnerId && cblolPo3.loserId && cblolPo4.loserId && !cblolPo8Match.t2) {
                    cblolPo8Match.t1 = cblolPo7.winnerId; cblolPo8Match.t2 = cblolPo3.loserId; isUpdated = true;
                }
                const cblolPo8 = simPlayoffMatch('cblol_po8');

                const cblolPo9Match = playoffs.find(m => m.id === 'cblol_po9');
                assignTeam(cblolPo9Match, cblolPo8.winnerId, cblolPo5.loserId);
                const cblolPo9 = simPlayoffMatch('cblol_po9');

                const cblolPo10Match = playoffs.find(m => m.id === 'cblol_po10');
                assignTeam(cblolPo10Match, cblolPo5.winnerId, cblolPo9.winnerId);
